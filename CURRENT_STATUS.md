...
- Phase UI-3D-B Settings V2 rollback/fallback foundation is implemented: Desktop Main now owns Settings V2 mount health, Renderer reports only fixed safe mount events, V2 renderer failure or mount timeout activates a session-only Legacy fallback, stale ready events cannot re-enable V2, and packaged Alpha still remains Legacy by release-channel gate.
