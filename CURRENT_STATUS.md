...
- Phase UI-3K-2C Single Read-Only Tool Result Re-entry is implemented for the sole internal `model.status` capability, mapped to provider function `model_status` with strict empty arguments. The existing registry and shared policy govern Core-owned in-memory status reads through the existing Task lifecycle; bounded success/failure results continue the same assistant turn, with one iteration, one tool, turn-scoped cancellation, and one final message. Formal-command tests and fake/local Desktop smoke pass with no real network requests. L4 integration is implemented; controlled real user acceptance is pending, so UI-3K-2C is not yet fully accepted. No filesystem tool or Windows action was added.
