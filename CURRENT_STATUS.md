...
- Phase UI-3K-2D Single Bounded Desktop Action has a real implementation wired into the formal UI, with L4 acceptance pending one user-operated Windows/DeepSeek run. Each action turn exposes only `local_app_open`, fixed to internal `localApp.open` with strict `{app:"notepad"}` arguments; existing `model.status` turns retain their separate sole definition. Deterministic Notepad requests and Assistant proposals reuse the existing registry policy, persisted Planner Approval/digest validation, and Task lifecycle. Desktop Main consumes a matching explicit UI task-approval grant and owns fixed System32 Notepad launch with empty arguments, no shell, and launched-PID verification. The old direct Notepad launch port fails closed without that approval context. Generic Windows policy remains disabled; a Notepad-only policy switch, cancellation, timeout, stale-result suppression, and one-final ownership bound the new path. Controlled formal-entry tests and fake Desktop smoke cover the loop with zero real provider requests and zero real Windows actions. Real acceptance and final evidence remain pending; UI-3K-2D is not fully accepted, and no second action, file write, browser, or L5 release claim is introduced.
