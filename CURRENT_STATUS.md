...
- Phase UI-3K-2D-Fix repairs the L3 bounded desktop implementation from `51f7c5b5fb2dae5faf37d6ae923980c2dfa4c3a9`; user-operated DeepSeek/Windows Deny and Allow acceptance is still pending. Ordinary conversational Assistant turns declare the existing `model_status` and `local_app_open` functions together. Only a raw provider tool call can select the normalized proposal through the fixed name mapping; no recording keywords, forced phrases, or upstream desktop intent are required. `localApp.open` still accepts only strict `{app:"notepad"}` arguments, and each turn permits at most one proposal/iteration with parallel calls disabled. Both deterministic and Assistant paths reuse ToolRegistry/Safety, persisted Task lifecycle, and existing Approval/digest validation. Rejecting a pending Notepad approval consumes it before asynchronous persistence, ends the Task without executing, returns a bounded denied tool result to the same provider turn, and permits one final explanation; cancelling the entire turn still aborts continuation. The existing approval dialog's Cancel now performs that denial. Desktop Main retains the fixed empty-argument, no-shell launch and launched-PID verification; generic Windows authority remains disabled. Automated adapter, formal Core, state-machine and isolated fake Desktop tests exercise natural selection, denial/allow, stale approval, cancellation and single-final ownership with zero real provider requests and zero real Windows actions. No real acceptance evidence is generated: UI-3K-2D fully accepted remains NO, with no second action, file/content operation, browser, or L5 release claim.
