...
async function run() {
  if (!APPROVAL_GATE_ENVS.every((name) => process.env[name] === "1")) {
    block("GLM_STANDARD_HEALTH_APPROVAL_GATE_MISSING");
    return;
  }
  ...
  if (!status.credentialConfigured || !safeStorage.isEncryptionAvailable()) {
    block("GLM_STANDARD_HEALTH_SECURE_CREDENTIAL_MISSING");
    return;
  }
    ...
    transport: {
      send: async (request) => {
        if (report.requestCount >= 1) {
          throw new Error("GLM_STANDARD_HEALTH_REQUEST_LIMIT_EXCEEDED");
        }
        if (request.url !== GLM_STANDARD_PAAS_V4_ENDPOINT) {
          throw new Error("GLM_STANDARD_HEALTH_ENDPOINT_MISMATCH");
        }
  ...
  report = {
    ...
    reasonCodes:
      result.diagnosticStatus === "healthy"
        ? ["GLM_STANDARD_HEALTHY"]
        : [`GLM_STANDARD_HEALTH_${result.diagnosticStatus.toUpperCase()}`]
  };
  if (!report.accepted) {
    process.exitCode = 1;
  }
...
async function finalizeCredentialCleanup() {
  ...
  try {
    ...
    if (!cleared) {
      markCleanupFailed("GLM_STANDARD_HEALTH_CLEANUP_VERIFICATION_FAILED");
      return;
    }
    report.cleanup = "complete";
  } catch {
    markCleanupFailed("GLM_STANDARD_HEALTH_CLEANUP_FAILED");
  }
}
...
function failureReasonCodeFor(stage) {
  return (
    {
      runtime_diagnostic: "GLM_STANDARD_HEALTH_RUNTIME_DIAGNOSTIC_FAILED",
      preflight: "GLM_STANDARD_HEALTH_PREFLIGHT_FAILED"
    }[stage] ?? "GLM_STANDARD_HEALTH_DIAGNOSTIC_FAILED"
  );
}

function createInitialReport() {
  return {
    scopeId: "glm-standard-paas-v4-health-diagnostic",
    status: "blocked",
    accepted: false,
