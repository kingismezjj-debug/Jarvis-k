...
const screenshotPath = path.join(
  artifactsDirectory,
  "jarvis-k-command-router-local-app-fixture-smoke.png"
);
const metricsPath = path.join(
  artifactsDirectory,
  "jarvis-k-command-router-local-app-fixture-smoke-metrics.json"
);
const smokeUserDataDirectory = await mkdtemp(
  path.join(os.tmpdir(), "jarvis-k-command-router-local-app-fixture-")
);
let electronApp;
...
try {
  ...
  await window.getByTestId("settings-command-router-qwen-binding")
    .getByText("Qwen Fast Router")
    .waitFor({ timeout: 5_000 });
  await window.getByTestId("settings-command-router-qwen-status")
    .getByText("disabled")
    .waitFor({ timeout: 5_000 });
  await window.getByTestId("settings-command-router-qwen-gates")
    .getByText("artifact digest")
    .waitFor({ timeout: 5_000 });
  ...
  await window
    ...
    .getByText("Confirm launch notepad")
    .waitFor({ timeout: 5_000 });
  ...
  await window
    ...
    .getByText("No Windows process was launched.")
    .waitFor({ timeout: 5_000 });
  ...
  if (newNotepadProcessIds.length > 0) {
    throw new Error(
      `Command Router fixture smoke launched notepad process(es): ${newNotepadProcessIds.join(", ")}.`
    );
  }
