...
try {
  ...
  if (boundedAction) {
    ...
    const denyPending = await waitStatus(page, "awaiting_approval");
    ...
    assert.equal(denied.assistantTurn.proposals[0].approvalStatus, "denied");
  ...
  assert.equal(calls.filter(call => call.type === "stream").length, boundedAction ? 9 : 5);
  assert.equal(calls.filter(call => call.type === "tool_result_received").length, boundedAction ? 3 : 1);
