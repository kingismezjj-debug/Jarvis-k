...
const smokeUserDataDirectory = await mkdtemp(
  path.join(os.tmpdir(), "jarvis-k-qwen-bounded-local-usage-")
);
let electronApp;
let appWindow;
...
async function sendBrainCommand(window, text, expectedIntent, expectedSummaryPattern) {
...
try {
  ...
  if (!/Route limit\s+5/i.test(activePanel) || !/Route count\s+0/i.test(activePanel)) {
    throw new Error("bounded usage session did not start with the approved limit.");
  }
  ...
  if (!/Route limit\s+5/i.test(routedPanel) || !/Route count\s+5/i.test(routedPanel)) {
    throw new Error("bounded local usage route count was not visible.");
  }
  ...
  console.log(
    JSON.stringify({
      ...
      mainConversationRouteRequestCount: 5,
