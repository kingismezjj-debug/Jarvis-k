...
async function configureCredential() {
  ...
  const first = await readMaskedSecret("GLM API key: ");
  const second = await readMaskedSecret("Confirm GLM API key: ");
  if (first.length === 0 || first !== second) {
    throw new Error("CREDENTIAL_CONFIRMATION_FAILED");
  }

  return writeCredentialThroughSecureStore(
    validateTerminalCredential(first)
  );
}
...
function isSanitizedConfiguredStatus(value) {
  return (
    isRecord(value) &&
    value.providerId === "heavy-planner.glm" &&
