...
const STORE_SCRIPT_PATH = path.join(
  __dirname,
  "configure-chat-answer-glm-credential-store.cjs"
);
...
async function configureCredential() {
  ...
  const first = await readMaskedSecret("GLM Chat Answer API key: ");
  const second = await readMaskedSecret("Confirm GLM Chat Answer API key: ");
...
function writeCredentialThroughSecureStore(credential) {
  return new Promise((resolve, reject) => {
    ...
    child.once("spawn", () => {
      child.send(
        {
          type: "store-chat-answer-glm-credential",
          credential
        },
        (error) => {
...
function isSanitizedConfiguredStatus(value) {
  return (
    isRecord(value) &&
    value.providerId === "chat-answer.openai-compatible.glm" &&
