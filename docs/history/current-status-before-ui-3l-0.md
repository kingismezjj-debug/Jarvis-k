# Historical status snapshot — before UI-3L-0

Archived verbatim from baseline 388a4fc65e3a849b764200baa428e109767c210d. Statements below include superseded and contradictory conclusions. They are historical evidence, not current status. See [CURRENT_STATUS.md](../../CURRENT_STATUS.md).
...
- Phase UI-3K-2C Single Read-Only Tool Result Re-entry is accepted at L4 on implementation `c4a0d2eb67c4bb75857308a2ae7e46ed6dbae8e8`: the sole internal `model.status` capability maps to provider function `model_status` with strict empty arguments. The existing registry, shared policy, and Task lifecycle govern Core-owned in-memory status reads; bounded success/failure results continue the same turn with one iteration, one tool, cancellation coverage, and one final message. The user-operated DeepSeek loop passed, including human confirmation of visible progress and a single final bubble. Safe projections confirm one allowed proposal, one completed execution/task, result continuation, and one canonical final. Two provider requests are inferred for this single user turn, with zero automated real requests. Formal-command tests, fake/local Desktop smoke, and clean-environment verification pass. Sanitized evidence at `artifacts/ui-3k/single-tool-result-reentry/acceptance.json` separates human observations, runtime projections, and source/test inference. Final Jarvis/CoreHost processes and all four Run identity counts are zero. UI-3K-2C is fully accepted; no filesystem tool or Windows action was added, no next phase is started, and L5 release readiness is not claimed.
- UI-3K-2D fully accepted = YES: L4 user-accepted bounded desktop action, limited to Notepad. The accepted implementation is `83b0910d9ef1aa48cd9555d5221db41ba5ec313b`, following `51f7c5b5fb2dae5faf37d6ae923980c2dfa4c3a9` and `98fbbfe37c084ff54b8e464008f6b211f4ce527f`. The user confirmed both native Deny and Allow flows PASS: approval preceded execution, denial launched nothing and returned a bounded denied result, allowance opened Notepad once with successful verification and result re-entry, and each flow produced one final message without internal exposure or additional Windows side effects. The user closed the blank test instance and exited Jarvis. [Acceptance evidence](artifacts/ui-3k/bounded-notepad-action-acceptance/acceptance.json) separates userObserved, userProvidedSafeStatus, sourceAndTestSupported, inferred request counts, and read-only closure checks. Approximately four provider requests across the accepted flows are inferred_not_packet_captured; the one user-approved bounded Notepad launch is userObserved, not a machine-captured action count. The evidence-only closure does not restart Jarvis, repeat acceptance, call a real provider, execute Windows actions, or modify product logic. Arbitrary application launch, file writing, Shell, browser tools, and filesystem.search are not completed by this bounded acceptance. WindowsAutomation expansion still requires per-tool governance; wake word has not been entered. No L5 release claim or automatic next phase is authorized.
- Assistant turn persistence and crash recovery is implemented at L3 with synthetic automated validation; controlled manual crash-recovery acceptance is not yet performed. Core owns the append-only orchestration journal port and bounded recovery policy, implemented in the existing CoreHost Task SQLite database (migration 2 -> 3). Canonical Conversation/Message and Task/Approval ownership is unchanged. Execution start must be durable before the Desktop port, bounded result metadata before continuation, and the reserved canonical Message ID supports terminal reconciliation without a duplicate answer. Startup retires old Task approvals and classifies unfinished turns as before execution, awaiting approval, unknown execution result, or after tool result; it never calls a provider or replays an action. Recovery notices are separate localized system status, with no restored streaming bubble or approval controls. Invalid histories are quarantined without deletion, unknown database versions fail closed, and scans are deterministic and capped at 100 turns (32 events each). New automated tests use synthetic SQLite, fake providers and a fake Desktop executor only; `realNetworkRequestSent=false`, `realWindowsActionPerformed=false`. [Design and validation](docs/assistant-turn-persistence-recovery.md). Existing UI-3K-2D L4 acceptance remains historical; this recovery change makes no new L4/L5 claim. No additional tool, filesystem.search, provider, Windows action, voice/wake word, plugin/MCP or release work is entered. Stop after implementation push; do not start real crash acceptance automatically.
