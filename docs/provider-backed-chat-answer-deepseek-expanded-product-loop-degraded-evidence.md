# Provider-Backed Chat Answer DeepSeek Expanded Product Loop Degraded Evidence

Recorded: 2026-08-09

## Result

`degraded_cleanup_complete`

The first approved one-window provider-backed Chat Answer DeepSeek expanded
product loop ran and closed, but it was not accepted. Visible product behavior
matched the expected result for four of the six fixed inputs. Two benign
security-explanation inputs were incorrectly classified as blocked.
...
```json
{
  "scopeId": "provider-backed-chat-answer-deepseek-expanded-product-loop",
  "status": "degraded_cleanup_complete",
  "accepted": false,
  ...
  "cleanup": "complete",
  "reasonCodes": [
    "DEEPSEEK_CHAT_ANSWER_EXPANDED_PRODUCT_LOOP_RESULT_MISMATCH"
  ]
}
```
...
After this degraded result, offline prompt hardening was implemented:

- added answered few-shot examples for the two misclassified benign safety
  explanation prompts;
- clarified that benign questions about bounded results, safe answers,
  protections, safety defaults, provider runtime, or previous safe answers
  must be answered rather than blocked; and
...
This window is consumed and closed as degraded. Do not count it as accepted
expanded product loop evidence. A replacement rerun requires fresh exact-scope
