...
function fixture(){const f=facts();const o:any={scenario:'B',pendingAt:performance.now(),facts:vi.fn(async()=>({...f})),authorize:vi.fn(async()=>({result:'granted',challengeMatched:true})),resolve:vi.fn(async()=>({synthetic:true})),crash:vi.fn(async(_:any,c:any)=>{await c.guard();c.markDispatch();return {executed:true};}),verifyExit:vi.fn(async()=>{}),checkpoint:vi.fn(async()=>{}),close:vi.fn(async()=>{}),finalCounters:vi.fn(async()=>({...f})),publish:vi.fn(async()=>{})};return {f,o};}
...
describe('H8 precise failure mapping and monitored approval',()=>{
 ...
 it('reader rejection is input_error, not timeout',async()=>{const {o}=fixture();o.authorize=async()=>{throw Error('secret');};expect((await G.run(o)).localAuthorization).toBe('input_error');});
 it.each([
...
describe('H8 external-only wiring and foreground identity',()=>{
 ...
 it('calibration failure cannot create a B profile',async()=>{const {authorizeCrash}=await import('../../../tests/recovery/authorize-crash.mjs');
 const create=vi.spyOn(P,'create'),descriptors=[process.stdin,process.stdout].map(s=>Object.getOwnPropertyDescriptor(s,'isTTY'));
 try{for(const stream of [process.stdin,process.stdout])Object.defineProperty(stream,'isTTY',{value:true,configurable:true});
 vi.spyOn(Input,'authorize').mockResolvedValue({result:'foreground_unverified'});await expect(authorizeCrash(['B'])).rejects.toThrow();expect(create).not.toHaveBeenCalled();
 }finally{[process.stdin,process.stdout].forEach((s,n)=>{if(descriptors[n])Object.defineProperty(s,'isTTY',descriptors[n]!);else delete (s as any).isTTY;});}});
 it('outer CLI forwards the gate firstFailure without a fake native assertion',()=>{const s=fs.readFileSync(path.join(P.REPO,'tests/recovery/authorize-crash.mjs'),'utf8');expect(s).toContain('firstFailure:result.firstFailure');expect(s).not.toContain("D.failure('native_approval_projection','launch','assertion_failed',true,false)");expect(s.indexOf('const calibration=await selftest()')).toBeLessThan(s.indexOf("P.create('B')"));});
 it('CLI classifies missing TTY before profile creation',async()=>{const {authorizeCrash}=await import('../../../tests/recovery/authorize-crash.mjs');const descriptor=Object.getOwnPropertyDescriptor(process.stdin,'isTTY');const create=vi.spyOn(P,'create');try{Object.defineProperty(process.stdin,'isTTY',{value:false,configurable:true});await expect(authorizeCrash(['B'])).rejects.toMatchObject({failure:{assertion:'local_authorization_channel',expected:'open',actual:'tty_unavailable'}});expect(create).not.toHaveBeenCalled();}finally{if(descriptor)Object.defineProperty(process.stdin,'isTTY',descriptor);else delete (process.stdin as any).isTTY;}});
