...
function gate(m:any,f:any){const o:any={scenario:'B',pendingAt:0,now:()=>0,monitor:m,authorize:async()=>grant(),resolve:vi.fn(async()=>({})),crash:vi.fn(async(_:any,c:any)=>{await c.guard();c.markDispatch();return require('../../../tests/recovery/controller-fixture.cjs').success();}),verifyExit:vi.fn(),close:vi.fn(),checkpoint:vi.fn(),finalCounters:async()=>({...f}),publish:vi.fn()};return o;}
