...
type UserControlledMemoryFilter = "all" | UserControlledMemoryKind;
type UserControlledMemoryRiskFilter =
  | "all"
  | UserControlledMemoryRecord["risk"];
type UserControlledMemorySort = "updated_desc" | "updated_asc" | "kind";
type UserControlledMemoryViewPreferences = {
  filter: UserControlledMemoryFilter;
  riskFilter: UserControlledMemoryRiskFilter;
  sort: UserControlledMemorySort;
};
type SanitizedUserControlledMemorySnapshotRecord = Pick<
  UserControlledMemoryRecord,
  | "id"
  | "kind"
  | "label"
  ...
  | "updatedAt"
>;
type SanitizedUserControlledMemorySnapshot = {
  schemaVersion: 1;
  generatedAt: string;
...
function formatUserControlledMemoryKey(
...
const userControlledMemoryFilterOptions: Array<{
...
const userControlledMemoryRiskFilterOptions: Array<{
...
const userControlledMemorySortOptions: Array<{
...
const LANGUAGE_STORAGE_KEY = "jarvis-k-ui-language";
const THEME_STORAGE_KEY = "jarvis-k-ui-theme";
const USER_CONTROLLED_MEMORY_VIEW_STORAGE_KEY =
  "jarvis-k-user-controlled-memory-view-v1";
const USER_CONTROLLED_MEMORY_SNAPSHOT_SCHEMA_VERSION = 1;
const defaultUserControlledMemoryViewPreferences: UserControlledMemoryViewPreferences =
  {
    filter: "all",
    riskFilter: "all",
    sort: "updated_desc",
...
const builtInSkinThemeIds = new Set<SkinThemeId>(
...
const uiCopy = {
  en: {
    ...
    label: {
      ...
      commandPlaceholder: "Send a typed command to Agent Core",
  ...
  zh: {
    ...
    action: {
      ...
      memorySnapshotExported: "Memory 快照已导出",
      memorySnapshotImported: "Memory 快照已导入",
      pasteMemorySnapshotFirst: "请先粘贴 Memory 快照 JSON",
      enterRecallProbeFirst: "请先输入 recall probe 文本",
      memoryAlphaRecallProbeComplete: "Memory Alpha recall probe 已完成",
      memoryAlphaIs: "Memory Alpha 当前状态为",
      memoryAlphaDisabled: "Memory Alpha 已禁用",
      ...
      chatAnswerProductModeRefreshed: "Chat Answer mode refreshed",
      chatAnswerProductModeChanged: "Chat Answer mode changed",
      memoryAlphaRefreshed: "Memory Alpha 已刷新",
    ...
    label: {
      ...
      noCoreTasks: "当前没有 Core 任务。",
      noModelOperations: "当前没有可见的模型生命周期操作。",
      noPlugins: "当前没有可见的内置或本地插件 manifest。",
      ...
      sendCommand: "发送命令",
      commandPlaceholder: "向 Agent Core 发送文本命令",
      recallProbePlaceholder: "Recall probe",
      memorySnapshotPlaceholder: "Memory 快照 JSON",
      agentCore: "AGENT CORE",
...
function isSkinThemeId(value: string | null): value is SkinThemeId {
  return builtInSkinThemeIds.has(value as SkinThemeId);
}

function readInitialSkinTheme(): SkinThemeId {
...
function isUserControlledMemoryFilter(
  value: unknown,
...
function isUserControlledMemoryRiskFilter(
  value: unknown,
...
function isUserControlledMemorySort(
  value: unknown,
): value is UserControlledMemorySort {
  return userControlledMemorySortOptions.some((option) => option.id === value);
}

function readInitialUserControlledMemoryViewPreferences(): UserControlledMemoryViewPreferences {
...
function persistUserControlledMemoryViewPreferences(
...
function buildSanitizedUserControlledMemorySnapshot(
...
function validateSanitizedUserControlledMemorySnapshot(
...
function isTaskCancellationEligible(state: TaskState): boolean {
  return (
    state === "queued" ||
    state === "planning" ||
...
function isTaskApprovalEligible(state: TaskState): boolean {
...
function formatPttCommandError(error: {
  code: string;
  details?: unknown;
  message: string;
}) {
...
function isSecondaryVoiceStopError(
...
function commandRouterAllowedRealLocalAppTarget(
  target: string,
...
function formatVoiceCorrectionSlots(slots: Record<string, unknown>) {
...
function formatVoiceCorrectionSlotValue(value: unknown) {
  if (
    typeof value === "string" ||
    typeof value === "number" ||
